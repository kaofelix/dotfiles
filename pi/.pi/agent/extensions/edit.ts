...
const editSchema = Type.Object({
  ...
  oldText: Type.Optional(
    Type.String({
      description:
        "Exact text to find and replace. Use with newText for one precise replacement.",
    }),
  ),
  ...
  edits: Type.Optional(
    Type.Array(
      ...
      {
        description:
          "Multiple exact replacements in one file. Prefer this for multiple precise edits in the same file.",
      },
    ),
  ),
});
...
function getReplaceModeInput(input: EditInput): ReplaceModeInput | null {
  ...
  if (oldText === undefined || newText === undefined) {
    throw new Error(
      "Edit tool input is invalid. Replacement mode requires both oldText and newText.",
    );
  }
  ...
  try {
    const output = await new Promise<string>(
      (resolvePromise, rejectPromise) => {
        const child = spawn(
          "git",
          ["diff", "--no-index", "--unified=4", "--", beforePath, afterPath],
          {
        ...
        child.on("close", (code) => {
          if (code === 0 || code === 1) {
            ...
            rejectPromise(
              new Error(stderr.trim() || `git diff failed with code ${code}`),
            );
          }
        });
      },
...
async function generateDiffString(
  ...
  for (const line of lines) {
    ...
    if (line.startsWith(" ")) {
      output.push(
        ` ${String(oldLineNum).padStart(lineNumWidth, " ")} ${line.slice(1)}`,
      );
      oldLineNum++;
      newLineNum++;
      continue;
    ...
    if (line.startsWith("+")) {
      if (firstChangedLine === undefined) firstChangedLine = newLineNum;
      output.push(
        `+${String(newLineNum).padStart(lineNumWidth, " ")} ${line.slice(1)}`,
      );
      newLineNum++;
    }
  }
...
async function executeMultiReplaceMode(
  ...
  return withFileMutationQueue(absolutePath, async () => {
    const { normalizedContent, bom, originalEnding } = await readNormalizedFile(
      absolutePath,
      signal,
    );
    ...
    for (const edit of normalizedEdits) {
      if (edit.oldText.length === 0) {
        throw new Error(
          "Edit tool input is invalid. edits[].oldText must not be empty.",
        );
      }
    ...
    for (let i = 1; i < matchedEdits.length; i++) {
      ...
      if (previous.index + previous.matchLength > current.index) {
        throw new Error(
          "Edit tool input is invalid. edits must not overlap in the original file.",
        );
      }
    }
    ...
    for (let i = matchedEdits.length - 1; i >= 0; i--) {
      ...
      newContent =
        newContent.slice(0, edit.index) +
        edit.newText +
        newContent.slice(edit.index + edit.matchLength);
    }
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      throw new Error(
        "Edit tool input is invalid. Provide either oldText and newText, or edits.",
      );
    },
  });
}
