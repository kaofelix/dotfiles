...
| `last_message` | Read the latest assistant text directly from a settled Pi agent's reported session |
...
Read a settled Pi agent's result from its native session rather than its terminal screen:
...
Use `read` when inspecting live terminal output or an agent kind without a readable Pi session:
...
Last-message and terminal-read output is truncated to the last 2,000 lines or 50KB, whichever is reached first.

For a settled Pi agent with a reported session path, prefer `last_message`; it does not depend on terminal dimensions or alternate-screen history. Full-screen agents may render terminal history through the alternate screen, where rows that leave the screen do not enter Herdr's host scrollback. For other agents, if increasing `read` lines does not reveal the complete response, ask the agent to write its response to a temporary Markdown file and read that file directly.
