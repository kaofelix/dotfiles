...
function assistantTextFromSessionLine(line: string): string | undefined {
	let entry: unknown;
	try {
	...
	if (typeof message.content === "string") return message.content.trim() ? message.content : undefined;
...
async function readLastAssistantMessage(sessionPath: string, signal?: AbortSignal): Promise<string> {
	let lastMessage: string | undefined;
	const lines = createInterface({ input: createReadStream(sessionPath, { encoding: "utf8" }), crlfDelay: Infinity });
	try {
		for await (const line of lines) {
			if (signal?.aborted) throw new Error("Aborted");
			lastMessage = assistantTextFromSessionLine(line) ?? lastMessage;
		}
	} finally {
		lines.close();
	}
	if (!lastMessage) throw new Error("No assistant text message found in the reported Pi session.");
	return lastMessage;
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Herdr Agent",
		description:
			"Control a recognized coding agent occupying an existing Herdr pane. Starting requires an available interactive shell pane created through herdr_layout and never creates or changes layout. Agent targets are unique live names or the pane ID currently hosting the agent, never terminal IDs or bare kind labels. Use prompt, wait, last_message, read, and send_keys instead of raw pane input. Lifecycle states are working, blocked, done, idle, and unknown; prompt and wait default to the first settled idle, done, or blocked state. Pi last-message and terminal-read output is truncated to 2000 lines or 50KB.",
		...
		promptGuidelines: [
			...
			"For normal helper work, create the requested Herdr topology, then use herdr_agent start, herdr_agent prompt with wait enabled, and herdr_agent last_message for a settled Pi agent. Use herdr_agent read for live terminal inspection and herdr_pane only for ordinary processes or intentional raw terminal control.",
			...
			"If herdr_agent last_message is unavailable for the agent kind and read cannot recover a full alternate-screen response after increasing lines, ask the agent to write its complete response to a temporary Markdown file and return the path, then read that file directly.",
		],
		parameters: Type.Object({
			action: StringEnum(["list", "get", "start", "prompt", "wait", "last_message", "read", "send_keys", "focus", "rename"] as const, {
		...
		async execute(_toolCallId, params, signal, onUpdate, _ctx) {
			switch (params.action) {
				...
				case "last_message": {
					if (!params.target) throw new Error("'target' is required for last_message");
					const response = await execHerdrJson<{ result: { agent: AgentInfo } }>(["agent", "get", params.target], signal);
					const agent = response.result.agent;
					if (agent.agent_status !== "idle" && agent.agent_status !== "done") {
						throw new Error(`Agent ${agentDisplayName(agent)} is ${agent.agent_status}; wait for idle or done before reading its last message.`);
					}
					const session = agent.agent_session;
					if (!session || session.kind !== "path" || session.agent !== "pi") {
						throw new Error(`Agent ${agentDisplayName(agent)} does not expose a readable Pi session path.`);
					}
					const message = await readLastAssistantMessage(session.value, signal);
					return {
						content: [{ type: "text", text: formatOutput(message) }],
						details: { action: "last_message", target: params.target, agent, read: true, source: "pi-session" },
					};
				}
				case "read": {
