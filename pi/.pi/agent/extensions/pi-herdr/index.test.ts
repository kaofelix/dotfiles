...
describe("pi-herdr", () => {
	...
	test("reads the last assistant message from a Pi agent session", async () => {
		...
		await writeFile(
			...
			[
				...
				{ type: "message", message: { role: "assistant", content: [{ type: "text", text: "Earlier reply" }] } },
				{
					type: "message",
					message: {
						role: "assistant",
		...
		try {
			...
			const result = await tools.get("herdr_agent").execute(
				"test",
				{ action: "last_message", target: "reviewer" },
				undefined,
				undefined,
				{},
			);

			expect(calls).toEqual([["agent", "get", "reviewer"]]);
