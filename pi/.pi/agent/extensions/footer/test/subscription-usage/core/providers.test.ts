...
import { AnthropicProvider } from "../../../src/subscription-usage/core/src/providers/impl/anthropic.js";
import { CopilotProvider } from "../../../src/subscription-usage/core/src/providers/impl/copilot.js";
import { GeminiProvider } from "../../../src/subscription-usage/core/src/providers/impl/gemini.js";
import { AntigravityProvider } from "../../../src/subscription-usage/core/src/providers/impl/antigravity.js";
import { CodexProvider } from "../../../src/subscription-usage/core/src/providers/impl/codex.js";
import { KimiProvider } from "../../../src/subscription-usage/core/src/providers/impl/kimi.js";
import { KiroProvider } from "../../../src/subscription-usage/core/src/providers/impl/kiro.js";
import { ZaiProvider } from "../../../src/subscription-usage/core/src/providers/impl/zai.js";
import { createDeps, createJsonResponse, getAuthPath } from "./helpers.js";
import type { UsageSnapshot } from "../../../src/subscription-usage/core/src/types.js";
...
test("kimi fetches weekly and 5h usage with a coding plan API key", async () => {
	...
	const { deps } = createDeps({
		env: { KIMI_CODING_API_KEY: "kimi-token" },
		fetch: async (url, init) => {
			requestedUrl = String(url);
			authorization = getHeader(init, "Authorization");
			return createJsonResponse({
				usage: { used: 25, limit: 100, resetTime },
				limits: [
					{
						window: { duration: 300, timeUnit: "MINUTE" },
						detail: { remaining: 80, limit: 100, resetTime },
					},
				],
			});
		},
	...
	assert.equal(usage.windows[1]?.resetAt, resetTime);
});

test("kimi parses numeric quota values returned as strings", async () => {
	const provider = new KimiProvider();
	const { deps } = createDeps({
...
test("kimi reads a static key from pi auth.json", async () => {
	const provider = new KimiProvider();
	let authorization: string | undefined;
	const { deps, files } = createDeps({
		fetch: async (_url, init) => {
			authorization = getHeader(init, "Authorization");
			return createJsonResponse({ usage: { used: 0, limit: 100 } });
		},
	});
	withAuth(files, { "kimi-coding": { type: "api_key", key: "pi-kimi-token" } }, deps.homedir());

	await provider.fetchUsage(deps);
	assert.equal(authorization, "Bearer pi-kimi-token");
});

test("kimi reads a static key from Kimi CLI config", async () => {
	const provider = new KimiProvider();
	let authorization: string | undefined;
	const { deps, files } = createDeps({
		fetch: async (_url, init) => {
	...
	files.set(
		`${deps.homedir()}/.kimi/config.json`,
		JSON.stringify({
			providers: {
				"kimi-for-coding": {
					type: "kimi",
					base_url: "https://api.kimi.com/coding/v1",
					api_key: "cli-kimi-token",
				},
			},
		})
	);
	...
	await provider.fetchUsage(deps);
	assert.equal(authorization, "Bearer cli-kimi-token");
});
