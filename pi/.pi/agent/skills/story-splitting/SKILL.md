...
This Pi adaptation is derived from Paul Hammond's MIT-licensed skill at [`citypaul/.dotfiles`](https://github.com/citypaul/.dotfiles/tree/main/claude/.claude/skills/story-splitting), fetched from commit `6220d843058ff33bb7d3dd3af175fd82b1c7965d`. It removes assumptions about companion Claude skills while preserving the standalone story-splitting method.
...
## How This Fits With The Workflow

This skill is self-contained and does not require companion skills. Use it before implementation planning when the input is a large story, epic, feature idea, roadmap item, or backlog item. It discovers small, valuable child stories; implementation planning then sequences one selected child story into technical delivery steps.

Keep story boundaries vertical regardless of PR topology. Database, API, UI, tests, documentation, and deployment may be tasks within a child story, but they are not separate backlog outcomes. Decide branch and PR structure only after selecting a child story, based on actual implementation dependencies.

If the real problem is an unresolved product or design decision, ask one high-value question at a time until the decision tree is resolved or the open decision is explicit. If a split already exists but has missing states, edge cases, acceptance criteria, or unverifiable wording, harden that artifact using the validation checklist below; split again if it remains horizontal or too large.

This skill must not drive implementation directly. Once a child story is selected:

1. Create an implementation plan for that one story, including evidence and release constraints.
2. Use the available `test-driven-development` skill for behavior-changing application work.
3. Use `safe-refactoring` instead when the work is strictly behavior-preserving structural cleanup.
4. Choose independent or stacked PRs from implementation dependencies; never turn technical layers into separate user stories.

## Requirement Refinement Pipeline

Use the earliest activity that matches the uncertainty:

| If the problem is... | Do... | Stop when you have... |
|----------------------|-------|------------------------|
| "We don't know which decision branch is right." | Ask one decision question at a time | A resolved decision tree or a named open decision |
| "This requirement is too broad or solution-shaped." | Apply this story-splitting workflow | Independently valuable child stories |
| "This story/spec/plan/mock has holes." | Validate and tighten the artifact | Testable wording, explicit states, and visible deferrals |
| "We selected a child story and need to build it." | Plan that child story, then use the appropriate implementation skill | Technical delivery steps with evidence |

Do not use story splitting to interrogate every product decision from scratch or to produce technical component tasks. Its output is a set of candidate child stories and a recommended first slice.
...
When asked to split a story, return a compact artifact for selecting a child story and beginning implementation planning:
...
## Next Step
[Usually: select the first slice, validate its acceptance examples and deferrals, then create an implementation plan for that slice. If a product decision still blocks the split, ask one high-value question. For implementation, use `test-driven-development` for behavior changes or `safe-refactoring` for strictly behavior-preserving cleanup.]
```
